...
object Graphics {
  ...
  case class BoxWithText(text:String,tx:Double, ty:Double,bx:Double, by:Double, bwidth:Double, bheight:Double)extends GraphicalPrimitive
  ...
  def translate(primitive:GraphicalPrimitive, dx:Double, dy:Double):GraphicalPrimitive = {
    primitive match {
      ...
      case bwt:BoxWithText => bwt.copy(bwt.text,bwt.tx+dx,bwt.ty+dy,bwt.bx+dx,bwt.by+dy)
