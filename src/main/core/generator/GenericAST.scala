  ...
  * */
  ...
  trait VarRefT extends ExpressionT {
    ...
    override def print(): Doc = {
      ...
      val vectorAccessD = vectorIndex match {
        case None     ⇒ empty
        case Some(ix) ⇒ ".s" <> ix.toHexString
      }
      ...
      v.print <> accessD <> suffixD <> vectorAccessD
  ...
  trait StringTypeT extends PrimitiveTypeT
  case class StringType(name:String) extends StringTypeT {
    override def _visit(pre: AstNode => Unit, post: AstNode => Unit): Unit = ()
    override def _visitAndRebuild(pre: (AstNode) => AstNode, post: (AstNode) => AstNode): AstNode = this
  }
  object StringType {
    def apply() = new StringType("std::string")
  }
