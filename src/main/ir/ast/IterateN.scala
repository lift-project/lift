...
/**
  ...
  */
case class IterateN(n: ArithExpr, f: Lambda) extends Pattern(arity = 1)
  with FPattern
  with isGenerable {
  ...
  override def copy(f: Lambda): Pattern = IterateN(n, f)
...
object IterateN {
  def apply(n: ArithExpr): ((Lambda1) => IterateN) =
    (f: Lambda1) => IterateN(n ,f)
