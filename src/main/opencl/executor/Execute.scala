...
import opencl.generator.{NDRange, OpenCLGeneratorNew, Verbose}
import opencl.ir._
...
class Execute(val localSize1: ArithExpr, val localSize2: ArithExpr, val localSize3: ArithExpr,
  ...
  private def createMemArgs(f: Lambda,
    ...
    OpenCLGeneratorNew.getMemories(f)._2.map(mem => {
  ...
  private def createSizeArgs(f: Lambda,
    ...
    val allVars = OpenCLGeneratorNew.getMemories(f)._2.map(
      _.mem.size.varList
