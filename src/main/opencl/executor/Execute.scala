...
import opencl.generator.{NDRange, OpenCLGeneratorOld, Verbose}
import opencl.ir._
...
class Execute(val localSize1: ArithExpr, val localSize2: ArithExpr, val localSize3: ArithExpr,
  ...
  private def createMemArgs(f: Lambda,
    ...
    OpenCLGeneratorOld.getMemories(f)._2.map(mem => {
  ...
  private def createSizeArgs(f: Lambda,
    ...
    val allVars = OpenCLGeneratorOld.getMemories(f)._2.map(
      _.mem.size.varList
