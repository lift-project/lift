      ...
      for(i <- 0 to (nx-1))
      {
        for (j <- 0 to (nx - 1))
        {
          for (k <- 0 to (reuse.eval - 1))
          {
            ...
            (block: Block) += AssignmentExpression(VarRef(sSP.windowVar, suffix = s"_${idx}"), ViewPrinter.emit(inputMem.variable, call.args.head.view.access(k).access(j).access(i)))
          }
        }
      }
    }
