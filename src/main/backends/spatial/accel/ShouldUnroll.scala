  ...
  Expr.visit(lambda.body, _ => Unit, {
    case call: FunCall =>
      ...
      call.f match {
        ...
        case r: ReduceSeq           => if (SpatialMemory.containsScalarMemory(call.args(1).mem))
                                          r.shouldUnroll = true
        case ma: MapAccumSeq        => if (SpatialMemory.containsScalarMemory(call.args(1).mem))
                                          ma.shouldUnroll = true
