...
//import lift.arithmetic.simplifier.{SimplifyCeiling, SimplifyIfThenElse}
//import lift.arithmetic.{ArithExpr, Cst, Predicate, RangeAdd, Var}
...
//
//class CacheLinesAccessedSymbolic private(lambda: Lambda,
...
//  }
//
////  def collectUserFunGlobalArgs(e: Expr): Seq[(UserFun, Seq[Expr])] = {
////    Expr.visitWithState((Seq[(UserFun, Seq[Expr])](), false))(e, {
////      case (_, (collectedArgs, insideMapSeqVector @ true)) => (collectedArgs, true)
////
////      case (FunCall(m: MapSeqVector, _*), (collectedArgs, _)) =>
////        val argsWithinFVectorized = if (m.vectorPartNonEmpty)
////          collectUserFunGlobalArgs(m.fVectorized.body)
////        else Seq()
////
...
////
////
////      case (FunCall(uf: UserFun, args @ _*), (collectedArgs, _)) =>
////        collectedArgs :+ (uf, args.filter {
////          case arg if arg.addressSpace.containsAddressSpace(GlobalMemory) => true
////          case arg if deviceConfig.localMemIsGlobal && arg.addressSpace.containsAddressSpace(LocalMemory) => true
////          case _ => false
////        })
////      case (_, collectedArgs) => collectedArgs
////    })
////  }
//
//  /**
...
//                              arg: Expr,
//                              verbose: Boolean = false): ArithExpr = {
//    val cacheLinesReadPerQuadPerAccess: ArithExpr = getCacheLinesReadPerQuad(arg)
//
...
//  }
//
//  def getCacheLinesReadPerQuad(arg: Expr): ArithExpr = {
//
...
//    val firstQuadIndices: Seq[ArithExpr] = (0 until 4).flatMap(i =>
...
//          case ae => ae
//        }
//      )).sortWith(ArithExpr.isCanonicallySorted)
//
...
//
//
//  private def getQuadIterationCount(arg: Expr): ArithExpr = {
//    val nIterations: List[Either[ArithExpr, ArithExpr]] = iterators(arg).flatMap {
//      // Sequential iterator
//      case Var(_, r @ RangeAdd(_: Cst, _, _)) =>
//        List(Right(r.numVals))
//
//      // Local iterator
...
//        val parIterations = SimplifyIfThenElse(
//          Predicate(of.range.asInstanceOf[RangeAdd].stop, stop, Operator.<=),
//          of.range.asInstanceOf[RangeAdd].stop,
//          stop)
////        val seqIterations = Math.ceil(stop.evalDouble / parIterations).toLong
//        val seqIterations = SimplifyCeiling(stop /^ parIterations)
//        List(Left(parIterations), Right(seqIterations))
//
//      // Wrg iterator
//      case Var(_, r @ RangeAdd(of: OclFunction, stop, _)) if of.name.equals("get_group_id") =>
//        List(Right(stop)) // Math.min(wrgSizes(of.param), stop.evalInt).toLong
//
//      // Global iterator
//      case Var(_, r @ RangeAdd(of: OclFunction, stop, _)) if of.name.equals("get_global_id") =>
//        List(Right(stop)) // Math.min(wrgSizes(of.param) * localSizes(of.param), stop.evalInt).toLong
//    }
//
//    val (localIterations: List[ArithExpr], seqAndNonLocalParIterations: List[ArithExpr]) = nIterations.map {
...
//    SimplifyCeiling(
//      SimplifyCeiling(
//        localIterations.foldLeft[ArithExpr](Cst(1))(_*_) /^ 4) *
//        seqAndNonLocalParIterations.foldLeft[ArithExpr](Cst(1))(_*_))
//  }
//}
//
//object CacheLinesAccessedSymbolic {
//
...
//    new CacheLinesAccessedSymbolic(lambda,
