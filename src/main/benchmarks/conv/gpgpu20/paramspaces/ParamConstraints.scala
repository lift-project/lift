...
import benchmarks.conv.gpgpu20.paramspaces.ParamConstraints.sortConstraints
...
//  val params: Vector[Var] = Vector(param0)
  ...
  def isValid(substitutionTable: Map[Var, Cst]): Boolean = {
    ...
    val lhsWithValues = lhs.visitAndRebuild(ParameterRewrite.substitute(_, reducedSubstitutionTable))
    val rhsWithValues = rhs.visitAndRebuild(ParameterRewrite.substitute(_, reducedSubstitutionTable))
  ...
  def lower(substitutionTable: Map[Var, Cst]): ParamConstraint = {
    ...
    new ParamConstraint(
      ...
      comment = comment,
      params = params,
      lhs = lhs.visitAndRebuild(ParameterRewrite.substitute(_, reducedSubstitutionTable)),
      rhs = rhs.visitAndRebuild(ParameterRewrite.substitute(_, reducedSubstitutionTable)),
      predicate = predicate
    )
  }
}
