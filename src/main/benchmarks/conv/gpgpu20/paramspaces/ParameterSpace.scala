...
import benchmarks.conv.gpgpu20.paramspaces.ParameterSpace.EmptySpace
  ...
  private def pickParamValues(params: List[Var],
    ...
    params match {
      ...
      case param :: remainingParams =>
        ...
        val rangeSize = time("rangeSize", RangeValueGenerator.rangeSize(param.range.visitAndRebuild(substitute(_, generatedParams))))
...
object ParameterSpace {
  ...
  def getRandomValueInRange(param: Var, random: Random, loweredRange: lift.arithmetic.Range, rangeSize: Int): Cst = {
    ...
    RangeValueGenerator.generateSingleValue(loweredRange, randomIndexInRange, None)
  }
