...
/*
  ...
  private def generateOclFunCall(fc: FunCall) : Block = {
    ...
    val eventCVar = CVarWithType("event_"+fc.gid, ClassOrStructType("cl::Event"))
    //val eventDecl = VarDeclPure( eventCVar, eventCVar.t  )
//    val stderr_thread_block_config = RawCode("cerr << " + '"' + "thread block config: " + '"'
//      + "<< lift_global_0 << ',' << ' ' << lift_global_1 << ',' << ' ' << lift_global_2 << endl;")
    val stderr_thread_block_config = RawCode("cerr << " + '"' + "thread block config(local): " +
      (if (local_thread_setting != null && local_thread_setting.toString != "") "cl::NDRange("+local_thread_setting.toString+")"
      else "cl::NullRange") + "\" << endl << " + '"' + "thread block config(global): " +
      "cl::NDRange("+global_thread_setting.toString+")" + "\" << endl;")
...
//        StringConstant("cl::NDRange(lift_global_0,lift_global_1,lift_global_2)"), //global
//        StringConstant("cl::NullRange"), //local, let compiler choose
        StringConstant("NULL"),
        if(measurable.gpu_timer) UnaryExpression("&", VarRefPure(eventCVar)) else StringConstant("NULL")
      )
    ) )
