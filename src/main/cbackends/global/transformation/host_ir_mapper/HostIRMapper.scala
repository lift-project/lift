  ...
  def apply(lambda: Lambda, intermediateGlobalMemMap: Map[Lambda, Seq[TypedOpenCLMemory]]) : Lambda = {
    val lowered = lambda visitAndRebuild  (pre = {
      ...
      case fc@FunCall(cf:CPUFunc, args@_*) =>
        val new_funcall = FunCall(CPUFunContainer(cf, intermediateGlobalMemMap.getOrElse(cf.f, Seq())), args:_*)
        new_funcall.t = fc.t
        new_funcall
      case fc@FunCall(of:OclFunc, args@_*) =>
        val new_funcall = FunCall(OclFunContainer(of, intermediateGlobalMemMap.getOrElse(of.f, Seq())), args:_*)
        new_funcall.t = fc.t
