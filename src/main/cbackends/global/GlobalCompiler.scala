...
import core.generator.GenericAST.{Block, Comment, ExpressionStatement, FunctionCall, FunctionPure, RawCode, StringConstant, VoidType}
...
object GlobalCompiler{
  ...
  def ! (lambda: Lambda, path: String, files: List[String], headerCommentCL: Option[String] = None): Unit = {
      ...
      case _ => {
        ...
        val oclFuns = all_oclfunc_outline_targets_purified.map {
          case (filename: String, localSize, globalSize, lambdax: Lambda) =>
            ...
            val annotatedBlock = headerCommentCL match {
              case None => block
              case Some(cmt) =>
                val commentsAST = cmt.replace("\r", "").split("\n").map(Comment)

                (Block() ++ commentsAST.toVector) :++ block
            }

            val intermediateGlobalMem = CollectTypedOpenCLMemory(lambdax)._3

            (filename, annotatedBlock, lambdax, intermediateGlobalMem)
        }
        ...
        ocl_source_files.foreach(file => logger.info("Saved a kernel in " + file.path + "/" + file.file))
      }
    }
