...
import ir._
import ir.ast.{AbstractMap, AbstractPartRed, Array2DFromUserFunGenerator, Array3DFromUserFunGenerator, ArrayAccess, ArrayFromUserFunGenerator, Expr, FunCall, Get, IRNode, Iterate, Join, Lambda, Pad, Param, Slide, Split, Transpose, TransposeW, Unslide, UserFun, Value, Zip, transpose}
import ir.view._
...
object InputView {
  ...
  def generateInputView(node: IRNode, cont: IRNode => IRNode): IRNode = {
    node match {
      ...
      case fc@FunCall(s:Unslide, arg) => {
        cont( arg )

        fc.view = arg.view.unslide(s)
