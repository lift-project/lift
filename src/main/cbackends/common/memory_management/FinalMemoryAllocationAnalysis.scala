...
import ir.ast.{AbstractMap, AbstractPartRed, Array3DFromUserFunGenerator, ArrayAccess, ArrayFromUserFunGenerator, FunCall, Get, IRNode, Iterate, Join, Lambda, Pad, Param, Slide, Split, Transpose, TransposeW, Unslide, UserFun, Zip}
...
object FinalMemoryAllocationAnalysis {
  ...
  def analyze(node:IRNode) : Map[String, (CVarWithType, ArithExpr, OpenCLAddressSpace)] = {
    node match {
      ...
      case fc@FunCall(_:Join|_:Slide|_:Unslide|_:Zip|_:Get|_:Split|_:Join|_:Transpose|_:TransposeW|_:Pad |_:ToLCP | _:ToGPE | _:ArrayAccess | _:Slice, args@_*) =>
        //args.foreach(analyze(_))
        args.map(analyze(_)).reduce( _ ++ _ )
