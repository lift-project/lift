...
import ir.ast.{AbstractMap, AbstractPartRed, ArrayAccess, ArrayConstructors, Expr, FPattern, FunCall, FunDecl, Get, IRNode, Join, Lambda, Pad, Param, Slide, Split, Transpose, TransposeW, Unslide, UserFun, Value, Zip}
  ...
  def alloc(node:IRNode, cont: IRNode => IRNode): IRNode = {
    node match {
      ...
      case fc@FunCall(l:Lambda, args@_*) => {
        ...
        //for Slide etc.
      case fc@FunCall(_:Join|_:Slide|_:Unslide|_:Zip|_:Get|_:Split|_:Join|_:Transpose|_:TransposeW|_:Pad|_:ArrayAccess|_:Slice, arg) => {
        cont(arg)
        fc.mem = arg.mem
