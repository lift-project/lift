...
package object nn {
  ...
  def validateResults(layerFactory: ConvStencil3D,
    ...
    def outputSize(tileSize: Var): Int = slidingOutputSize(
      substitutionTable(tileSize),
      substitutionTable(layerConfigVars.kernelWidthHeight),
      substitutionTable(layerConfigVars.kernelStride)).evalInt *
      slidingOutputSize(
    ...
    val result = patterns.nn.group(liftResult, (substitutionTable(layerConfigVars.nInputs).evalInt,
      ...
      outputSize(tuneParamVars.tileWidth), outputSize(tuneParamVars.tileHeight)))
  ...
  def parseCSV(csvPath: String): collection.mutable.Map[Int, LayerPoints] = {
    ...
    val params = List(
      ...
      "nKernelsPerWrg", "nWindowsPerThread", "seqOpsPerThread", "tileWidthHeight", "padOptX", "padOptY",
    ...
    for (line <- lineIterator) {
      ...
      result(layerConfigIdx).tuningPoints += TuningPoint(
        ...
        values = scala.Vector(
          ...
          /* padOptX */ Cst(values(paramColumns("padOptX")).toInt),
          /* padOptY */ Cst(values(paramColumns("padOptY")).toInt)
        ),
        rewriteRules = {
          ...
          // TODO: update to handle parametrised vectorisation
