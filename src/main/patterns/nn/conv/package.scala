...
package object conv {
  def generateTestData(convLambdaFactory: ConvStencil3D,
                       ...
                       inputWidth: Int, inputHeight: Int): (Array4D[Float], Array[Float], Array4D[Float]) = {
    ...
    val x: Array4D[Float] = Array.tabulate(
      ...
      inputHeight, //substitutionTable(convStencil3DLayerConfig.inputWidthHeight).evalInt,
      inputWidth, //substitutionTable(convStencil3DLayerConfig.inputWidthHeight).evalInt,
      substitutionTable(convStencil3DLayerConfig.inputChannels).evalInt)((_, _, _, _) => Random.nextFloat())
