...
object ParameterRewrite {
  ...
  val parser = {
    ...
    OParser.sequence(
      ...
      opt[Unit]('e', keyExploreNDRange).text(
        s"Explore global and local sizes instead of inferring (default: $defaultExploreNDRange)")
        .action((_, c) => c.copy(exploreNDRange = true)),

      opt[Int]('n', keySampleNDRange).text(s"Randomly sample n combinations of global and local sizes " +
        s"(requires 'explore') (default: $defaultSampleNDRange)")
        .action((arg, c) => c.copy(sampleNDRange = arg)),

      opt[Unit](keyDisableNDRangeInjection).text(
      ...
      opt[Unit](keyGenerateScala).text(s"Generate lambdas in Scala as well as in OpenCL (default: $defaultGenerateScala)")
