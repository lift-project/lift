...
class TestPadding {
  ...
  def testBothLsOnBiggerExample(): Unit = {
    ...
    val paddingLambda = ParameterRewrite(patterns.nn.pad.Pad(layerConfigVars, tuneParamVars, factory,
      (substitutionTable(layerConfigVars.padFunc) + substitutionTable(tuneParamVars.padOpt)).evalInt),
      substitutionTable)

    val paddingLambdaNDRanges = patterns.nn.pad.Pad.paddingLambdaNDRanges(substitutionTable, layerConfigVars, factory)
    ...
    val gold = new PadConstant(0, 0, Value("0", opencl.ir.Float)).eval2d(X,
