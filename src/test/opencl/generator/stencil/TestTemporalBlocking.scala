 ...
 ***********************************************************/
  ...
  @Test def tempAndSpatialBlocking1D(): Unit = {
    ...
    //gold computation
    val (firstIteration: Array[Float], _) = ExecuteOld(length, length)(newLambda, randomData, weights)
    //val (gold: Array[Float], runtime3) = Execute(length, length)(newLambda, firstIteration, weights)

    val (secondIteration: Array[Float], _) = ExecuteOld(length, length)(newLambda, firstIteration, weights)
    val (_, _) = ExecuteOld(length, length)(newLambda, secondIteration, weights)
    //println(gold.mkString(","))

    val stencil = createTempAndSpatialBlockingLambda(3, 1, 4, 2, 1, 1, Pad.Boundary.Clamp)
    val (_, _) = ExecuteOld(length, length)(stencil, randomData)
  ...
  @Test def temporalBlockingUsingRewrite1D(): Unit = {
    ...
    val (secondIteration: Array[Float], _) = ExecuteOld(length, length)(newLambda, firstIteration, weights)
    val (gold: Array[Float], _) = ExecuteOld(length, length)(newLambda, secondIteration, weights)

    val stencil = createTemporalBlockingUsingRewriteLambda(Pad.Boundary.Clamp, 3, 1, 1, 1)
    val (output: Array[Float], runtime) = ExecuteOld(length, length)(stencil, randomData)
  ...
  @Test def temporalBlockingUsingTiles1D(): Unit = {
    ...
    val newLambda = create1DStencilLambda(weights, 3, 1, 1, 1)
    val (output: Array[Float], runtime) = ExecuteOld(length, length)(stencil, randomData, weights)
    val (firstIteration: Array[Float], _) = ExecuteOld(length, length)(newLambda, randomData, weights)
    val (gold: Array[Float], _) = ExecuteOld(length, length)(newLambda, firstIteration, weights)
