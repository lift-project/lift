...
class TestAcousticActualRoom {
  ...
  def test3DAsymMaskStencilWithAtAndTiling(): Unit = {
    ...
    val (output: Array[Float], runtime) = Execute(2,2,2,2,2,2, (true,true))(source,newLambda, stencilarrpadded3D, StencilUtilities.weightsMiddle3D.flatten.flatten)
  ...
  def roomCodeUsingSlideSeqPlus(): Unit = {
    ...
    val lambdaNeighAt = fun(
      ...
      (mat1, mat2) => {
        ...
        )) $ Zip2D(PrintType() $ mat1, PrintType() o Map(Map(Transpose())) o Map(Map(Map(Transpose()))) o Slide2D(StencilUtilities.slidesize, StencilUtilities.slidestep) $ mat2, PrintType() $ Array3DFromUserFunGenerator(getNumNeighbours, arraySig))
      })
