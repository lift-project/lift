    ...
    val stencil = fun(
      ...
      (temp, power) => {
        MapGlb(0)(MapGlb(1)(fun (x => { toGlobal(MapSeqSlide(fun((m) => {
          rodinia(m)
        }),size,step)) } o Transpose() o Map(Transpose()) $ x ))) $ Zip3D(power, Slide3D(size,step) o Pad3D(1, 1, 1, Pad.Boundary.Clamp) $ temp)
      })
    ...
    val acousticStencil =
      fun(
        ...
        (mat1, mat2) => {
            MapGlb(0)(MapGlb(1)(fun (x => { toGlobal(MapSeqSlide(fun((m) => {
            acoustic(m)
          }),size,step))} o Transpose() o Map(Transpose()) $ x ))) $ Zip3D(mat1, Slide3D(size, step) $ mat2, Array3DFromUserFunGenerator(getNumNeighbours, arraySig))
        })
