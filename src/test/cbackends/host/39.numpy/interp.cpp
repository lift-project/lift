...
int main(int argc, char *argv[])
{
	...
	vector<float> x {1,2,3}, y {3,2,0}, interp_x {1, 1.5, 2.72};
	float *r = nullptr;
	const int size = x.size();

	lift::interp(interp_x.data(), x.data(), y.data(), r,interp_x.size(), size);
	/* lift::cos(x.data(),y,5); */
	...
	system("./numpy/interp.py > numpy_golden_data/interp.txt");


	const int golden_data_size = size;
	vector<float> golden_data(golden_data_size, -999.99);

	ifstream file("numpy_golden_data/interp.txt");
	...
	for(int i = 0; i < golden_data_size; ++i)
	{
		...
		if( abs( golden_data[i] - r[i] ) > tol ){
			std::cout << "[interp]: Computed results does not match the golden data !!!" << std::endl;
			std::cout << "------------------------------------------------------------" << std::endl;
