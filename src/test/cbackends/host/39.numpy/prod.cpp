...
int main(int argc, char *argv[])
{
	...
	system("./numpy/prod.py > numpy_golden_data/prod.txt");


	const int golden_data_size = rsize;
	vector<float> golden_data(golden_data_size, -999.99);

	ifstream file("numpy_golden_data/prod.txt");
	...
	for(int i = 0; i < golden_data_size; ++i)
	{
		...
		if( abs( golden_data[i] - r[i] ) > tol ){
			std::cout << "[prod]: Computed results does not match the golden data !!!" << std::endl;
			std::cout << "------------------------------------------------------------" << std::endl;
