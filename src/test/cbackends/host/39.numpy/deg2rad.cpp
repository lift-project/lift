...
int main(int argc, char *argv[])
{
	...
	system("./numpy/deg2rad.py > numpy_golden_data/deg2rad.txt");


	const int golden_data_size = size;
	vector<float> golden_data(golden_data_size, -999.99);

	ifstream file("numpy_golden_data/deg2rad.txt");
	...
	for(int i = 0; i < golden_data_size; ++i)
	{
		...
		if( abs( golden_data[i] - y[i] ) > tol ){
			std::cout << "[deg2rad]: Computed results does not match the golden data !!!" << std::endl;
			exit(1);
		}
	}
