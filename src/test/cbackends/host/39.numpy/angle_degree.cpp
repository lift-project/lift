...
int main(int argc, char *argv[])
{
	...
	lift::angle_degree(x.data(), r,size);
	/* lift::cos(x.data(),y,5); */
	...
	system("./numpy/angle_degree.py > numpy_golden_data/angle_degree.txt");


	const int golden_data_size = size;
	vector<float> golden_data(golden_data_size, -999.99);

	ifstream file("numpy_golden_data/angle_degree.txt");
	...
	for(int i = 0; i < golden_data_size; ++i)
	{
		...
		if( abs( golden_data[i] - r[i] ) > tol ){
			std::cout << "[angle_degree]: Computed results does not match the golden data !!!" << std::endl;
			std::cout << "------------------------------------------------------------" << std::endl;
