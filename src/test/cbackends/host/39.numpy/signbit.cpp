...
int main(int argc, char *argv[])
{
	...
	vector<float> x {-1,2,-3,4,0};
	float *r = nullptr;
	const int size = x.size();

	lift::signbit(x.data(),r,size);
	/* lift::cos(x.data(),y,5); */
	...
	system("./numpy/signbit.py > numpy_golden_data/signbit.txt");


	const int golden_data_size = size;
	vector<float> golden_data(golden_data_size, -999.99);

	ifstream file("numpy_golden_data/signbit.txt");
	...
	for(int i = 0; i < golden_data_size; ++i)
	{
		...
		if( abs( golden_data[i] - r[i] ) > tol ){
			std::cout << "[signbit]: Computed results does not match the golden data !!!" << std::endl;
			std::cout << "------------------------------------------------------------" << std::endl;
