...
int main(int argc, char *argv[])
{
	...
	vector<float> x {1, 2, 4, 7, 11, 16}, y(x.begin(), x.end());
	float *r = nullptr;
	const int size = x.size();
	const int rsize = size - 2;
	assert(rsize > 0);

	lift::gradient(x.data(),r,size);
	/* lift::cos(x.data(),y,5); */

	//lift does not compute the first and last element
	//so need to compute manually, ideally at python level
	copy(r, r+rsize, y.begin() + 1);
	assert(size - 2 > 0);
	y[size-1] = x[size -1] - x[size -2];

	copy(y.begin(), y.end(), ostream_iterator<float>(cout, " "));
	std::cout << std::endl;

	system("mkdir -p numpy_golden_data");
	system("./numpy/gradient.py > numpy_golden_data/gradient.txt");


	const int golden_data_size = size;
	vector<float> golden_data(golden_data_size, -999.99);

	ifstream file("numpy_golden_data/gradient.txt");
	...
	for(int i = 0; i < golden_data_size; ++i)
	{
		...
		if( abs( golden_data[i] - y[i] ) > tol ){
			std::cout << "[gradient]: Computed results does not match the golden data !!!" << std::endl;
			std::cout << "------------------------------------------------------------" << std::endl;
