...
int main(int argc, char *argv[])
{
	...
	//notice x is two-dimensional, 2 by 2
	vector<float> x {0,1,0,5};
	float *r = nullptr;
	const int size = x.size();

	lift::sum_axis_0(x.data(), r, 2, 2);
	/* lift::cos(x.data(),y,5); */
	...
	system("./numpy/sum_axis_0.py > numpy_golden_data/sum_axis_0.txt");


	const int golden_data_size = 2;
	vector<float> golden_data(golden_data_size, -999.99);

	ifstream file("numpy_golden_data/sum_axis_0.txt");
	...
	for(int i = 0; i < golden_data_size; ++i)
	{
		...
		if( abs( golden_data[i] - r[i] ) > tol ){
			std::cout << "[sum_axis_0]: Computed results does not match the golden data !!!" << std::endl;
			std::cout << "------------------------------------------------------------" << std::endl;
