...
int main(int argc, char *argv[])
{
	...
	const int rsize = size - 1;

	lift::ediff1d(x.data(),r,size);
	/* lift::cos(x.data(),y,5); */
	...
	system("./numpy/ediff1d.py > numpy_golden_data/ediff1d.txt");


	const int golden_data_size = rsize;
	vector<float> golden_data(golden_data_size, -999.99);

	ifstream file("numpy_golden_data/ediff1d.txt");
	...
	for(int i = 0; i < golden_data_size; ++i)
	{
		...
		if( abs( golden_data[i] - r[i] ) > tol ){
			std::cout << "[ediff1d]: Computed results does not match the golden data !!!" << std::endl;
			std::cout << "------------------------------------------------------------" << std::endl;
