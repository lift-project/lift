...
int main(int argc, char *argv[])
{
	...
	vector<float> x {5,2,3,4}, y{2,2,4,5};
	float *r = nullptr;
	const int size = x.size();

	lift::nextafter(x.data(),y.data(),r,size);
	/* lift::cos(x.data(),y,5); */

	copy(r, r+size, ostream_iterator<float>(cout<<setprecision(9), " "));
	std::cout << std::endl;

	system("mkdir -p numpy_golden_data");
	system("./numpy/nextafter.py > numpy_golden_data/nextafter.txt");


	const int golden_data_size = size;
	vector<float> golden_data(golden_data_size, -999.99);

	ifstream file("numpy_golden_data/nextafter.txt");
	...
	const float tol = 0.000000000000001;

	for(int i = 0; i < golden_data_size; ++i)
	{
		/* std::cout << abs( golden_data[i] - y[i] ) << std::endl; */
		if( abs( golden_data[i] - r[i] ) > tol ){
			std::cout << "[nextafter]: Computed results does not match the golden data !!!" << std::endl;
			std::cout << "------------------------------------------------------------" << std::endl;
