...
int main(int argc, char *argv[])
{
	...
	vector<float> x {3,3,3};
	vector<float> y {4,4,4};
	float *r = nullptr;
	const int size = x.size();

	lift::hypot(x.data(),y.data(), r ,size);
	/* lift::cos(x.data(),y,5); */
	...
	system("./numpy/hypot.py > numpy_golden_data/hypot.txt");


	const int golden_data_size = size;
	vector<float> golden_data(golden_data_size, -999.99);

	ifstream file("numpy_golden_data/hypot.txt");
	...
	for(int i = 0; i < golden_data_size; ++i)
	{
		...
		if( abs( golden_data[i] - r[i] ) > tol ){
			std::cout << "[hypot]: Computed results does not match the golden data !!!" << std::endl;
			std::cout << "------------------------------------------------------------" << std::endl;
