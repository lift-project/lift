...
int main(int argc, char *argv[])
{
	...
	system("./numpy/degrees.py > numpy_golden_data/degrees.txt");


	const int golden_data_size = size;
	vector<float> golden_data(golden_data_size, -999.99);

	ifstream file("numpy_golden_data/degrees.txt");
	...
	for(int i = 0; i < golden_data_size; ++i)
	{
		...
		if( abs( golden_data[i] - y[i] ) > tol ){
			std::cout << "[degrees]: Computed results does not match the golden data !!!" << std::endl;
			exit(1);
		}
	}
