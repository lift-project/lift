...
int main(int argc, char *argv[])
{
	...
	vector<float> x {1.2,1.7};
	float *r = nullptr;
	const int size = x.size();

	lift::rint(x.data(),r,size);
	/* lift::cos(x.data(),y,5); */
	...
	system("./numpy/rint.py > numpy_golden_data/rint.txt");


	const int golden_data_size = size;
	vector<float> golden_data(golden_data_size, -999.99);

	ifstream file("numpy_golden_data/rint.txt");
	...
	for(int i = 0; i < golden_data_size; ++i)
	{
		...
		if( abs( golden_data[i] - r[i] ) > tol ){
			std::cout << "[rint]: Computed results does not match the golden data !!!" << std::endl;
			std::cout << "------------------------------------------------------------" << std::endl;
