...
@Ignore("not ready yet")
@Test
def test_mm_with_host(): Unit = {

  val path = s"$common_path/18.mm_with_host"
  val file = "libgpufunc.cpp"

  ("mkdir -p " + s"$path" ) !!

  val mm : Lambda = fun( (A_, B_) =>
      MapGlb(1)( fun( a => MapGlb(0)( fun( b => toGlobal(MapSeq(id)) o ReduceSeq( fun( (acc, x) => multAndSumUp.apply(acc, Get(x,0), Get(x,1)) ), 0.0f ) $ Zip(a,b) ) ) $ B_) ) $ A_ )

  val f = fun(
  ArrayTypeWS(ArrayTypeWS(Float, K), M),
  ArrayTypeWS(ArrayTypeWS(Float, K), N),
    (A,B) => ToHost() $ OclFunc ( mm ).apply( ToGPU() $ A, ToGPU() $ B)
  )
  ...
  @Ignore("not ready yet")
  @Test
  def test_dmm_fusion(): Unit = {

    val path = s"$common_path/19.dmm_fusion"
    //val path_v1 = s"$path/1.nonfused"
    //val path_v2 = s"$path/2.fused"
    val file = "libfused.cpp"
    val file_cpu = "libfused_cpu.cpp"
    val file1 = "libdmm1.cpp"
    val file1_cpu = "libdmm1_cpu.cpp"
    /* val file2 = "libdmm2.cpp"
    val file2_cpu = "libdmm2_cpu.cpp" */
    ...
    val dmm : Lambda = fun(
      (A_, B_) =>  MapGlb(1)(fun( Arow =>
      Join() o  MapGlb(0)(fun( Bcol =>
        toGlobal(MapSeq(id)) o ReduceSeq(fun((acc, y) => multAndSumUp.apply(acc, Get(y, 0), Get(y, 1))), 0.0f) $ Zip(Arow, Bcol)
      )) o Transpose() $ B_
    )) $ A_ )
    ...
    val f = fun(
      ...
      ArrayTypeWSWC(ArrayTypeWSWC(Float, K), M),
      ArrayTypeWSWC(ArrayTypeWSWC(Float, N), K),
      (A,B) => ToHost(cpu_timer = true, gpu_timer = true) $ OclFunc ( dmm, cpu_timer = true, gpu_timer = true ).apply( ToGPU(cpu_timer = true, gpu_timer = true) $ A, ToGPU(cpu_timer = true, gpu_timer = true) $ B)
      )


    GlobalCompiler ! (f, path, List(file1))

    /*
     import opencl.executor.Compile

    val c1 = Compile(dmm_cpu)
    println("check_point")


    import java.io.PrintWriter
    new PrintWriter(path + "/" + file1) {
      write(c1); close
    }
    (s"$path/shape.sh $path/$file1") #| s"$path/shape1.sh $path/$file1_cpu" !
*/
    ...
    val g = fun(
      ...
      ArrayTypeWSWC(ArrayTypeWSWC(Float, n), o),
      (A, B) => {
        MapSeq(fun( Arow =>
          Join() o  MapSeq(fun( Bcol =>
            toGlobal(MapSeq(id)) o ReduceSeq(fun((acc, y) => multAndSumUp.apply(acc, Get(y, 0), Get(y, 1))), 0.0f) $ Zip(Arow, Bcol)
    ...
    import java.io.PrintWriter
    new PrintWriter(path + "/" + file2) {
      write(c2); close
    }

    (s"$path/shape.sh $path/$file2") #| s"$path/shape2.sh $path/$file2_cpu" !
    */

    val fused_dmm_cpu = fun(
      ArrayTypeWSWC(ArrayTypeWSWC(Float, K), M),
      ArrayTypeWSWC(ArrayTypeWSWC(Float, N), K),
      ArrayTypeWSWC(ArrayTypeWSWC(Float, O), N),
      (A_, B_, D_) => MapSeq( fun(a =>

        Let( temp =>

          MapSeq( fun(d =>

            toGlobal(MapSeq(id)) o ReduceSeq( fun((acc, y) => multAndSumUp.apply(acc, Get(y, 0), Get(y, 1))), 0.0f

            ) $ Zip( d, temp )

          )) o Transpose() $ D_

        ) o Join() o MapSeq( fun( b => toGlobal(MapSeq(id)) o ReduceSeq(fun((acc, y) => multAndSumUp.apply(acc, Get(y, 0), Get(y, 1))), 0.0f)$ Zip(a,b) )) o Transpose() $ B_
    ...
    val fused = fun(
      ...
      ArrayTypeWSWC(ArrayTypeWSWC(Float, N), K),
      ArrayTypeWSWC(ArrayTypeWSWC(Float, O), N),
      (A, B, D) => ToHost(cpu_timer = true, gpu_timer = true) $ OclFunc ( fused_dmm, cpu_timer = true, gpu_timer = true ).apply( ToGPU(cpu_timer = true, gpu_timer = true) $ A, ToGPU(cpu_timer = true, gpu_timer = true) $ B, ToGPU(cpu_timer = true, gpu_timer = true) $ D)
    )

    GlobalCompiler ! (fused, path, List(file))

    /*

    //TypeChecker(fused_dmm_cpu)
    //val clean_script = s"$path/clean.sh"
    //val f_test = fused.toString.replace('\n', '@').replace("  ", "£")
    //val f_print = ( "echo " + '"' + f_test + '"' ) #| s"$clean_script" !!

    //val lowered = ParametricRewriter.lowering(fused)

    val c = Compile(fused_dmm_cpu)
    println("check_point")

    import java.io.PrintWriter
    new PrintWriter(path + "/" + file) {
      write(c); close
    }

    s"$path/shape.sh $path/$file" #> new File(s"$path/$file_cpu") !

    //s"rm $path/$file $path/$file_cpu $path/$file1 $path/$file1_cpu $path/$file2 $path/$file2_cpu $path/a.out" !
    s"rm $path/$file $path/$file1 $path/a.out" !
    */
