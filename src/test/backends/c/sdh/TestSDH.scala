...
class TestSDH {
  ...
  val common_path = System.getProperty("user.dir") + "/src/test/backends.c/sdh"
  ...
  */
  ...
  def test_vec_add_multi_tile(): Unit = {
    ...
    SDHCompiler compile (f, path, List(sched_file, worker_file))
  ...
  def test_matrix_mul_sdh_demo(): Unit = {
    ...
    SDHCompiler compile (f, path, List(sched_file, worker_file), "matrixmul")
  ...
  def test_generate_all_numpy_functions(): Unit = {
    ...
    (func_names zip all_funcs).foreach {
      case (func_name, func) => SDHCompiler compile (func, path,
