...
int main(int argc, char *argv[])
{
	...
	lift::arctan2(x.data(),y.data(), r ,size);
	/* lift::cos(x.data(),y,5); */
	...
	system("./numpy/arctan2.py > numpy_golden_data/arctan2.txt");


	const int golden_data_size = size;
	vector<float> golden_data(golden_data_size, -999.99);

	ifstream file("numpy_golden_data/arctan2.txt");
	...
	for(int i = 0; i < golden_data_size; ++i)
	{
		...
		if( abs( golden_data[i] - r[i] ) > tol ){
			std::cout << "[arctan2]: Computed results does not match the golden data !!!" << std::endl;
			std::cout << "------------------------------------------------------------" << std::endl;
