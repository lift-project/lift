...
int main(int argc, char *argv[])
{
	...
	system("./numpy/tan.py > numpy_golden_data/tan.txt");


	const int golden_data_size = size;
	vector<float> golden_data(golden_data_size, -999.99);

	ifstream file("numpy_golden_data/tan.txt");
	...
	for(int i = 0; i < golden_data_size; ++i)
	{
		...
		if( abs( golden_data[i] - y[i] ) > tol ){
			std::cout << "[tan]: Computed results does not match the golden data !!!" << std::endl;
			exit(1);
		}
	}
