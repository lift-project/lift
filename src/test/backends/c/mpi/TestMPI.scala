...
class TestMPI {
  ...
  val common_path = {
    if (System.getProperty("user.name").equals("eduardo")) {
      "/home/eduardo/lift_bueno/lift/src/test/backends.c/mpi" }
    else {
      System.getProperty("user.dir") + "/src/test/backends.c/mpi" }
  }
  ...
  def test_mpi_bcast(): Unit = {
    ...
    MPICompiler compile (basicMPIBcast, path, List(file) )
