...
object TestMMExplicitPrivateCGO extends TestWithExecutor

class TestMMExplicitPrivateCGO {

  private val N = GEMM.N
  private val M = GEMM.M
  private val K = GEMM.K

  private val mSize = 1024
  private val kSize = 1024
  private val nSize = 1024

  private val matrixA = Array.tabulate(mSize, kSize)((r, c) => (((r * 3 + c * 2) % 10) + 1) * 1.0f)
  private val matrixB = Array.tabulate(kSize, nSize)((r, c) => (((r * 7 + c * 3) % 10) + 1) * 1.0f)
  private val matrixC = Array.tabulate(kSize, mSize)((r, c) => (((r * 7 + c * 3) % 10) + 1) * 1.0f)

  private val alpha = 1.5f
  private val beta = 2.5f
