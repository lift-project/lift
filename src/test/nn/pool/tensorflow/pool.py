...
class Pool:
    ...
    def conv_net_full(x, weights, biases, image_shape):
        ...
        # Convolution Layer
        conv1 = Pool.conv2d(x, weights['wconv1'], biases['bconv1'])
        # Max Pooling (down-sampling)
        #conv1 = CNN.maxpool2d(conv1, k=2)

        # Convolution Layer
        conv2 = Pool.conv2d(conv1, weights['wconv2'], biases['bconv2'])
        # Max Pooling (down-sampling)
    ...
    def __init__(self, n_kernels, kernel_shape, image_shape):
        ...
        # Construct model
        self.pred = Pool.conv_net_full(self.x, self.weights, self.biases, self.image_shape)  # , keep_prob)
        ...
        self.dir_name = Pool.get_dir_name(self.n_kernels, self.kernel_shape, self.image_shape)
        # Create directory
    ...
    def restore(n_kernels, kernel_shape, image_shape):
        ...
        :return:
        """
        dir_name = Pool.get_dir_name(n_kernels, kernel_shape, image_shape)
